...
# 带 ngrok 公网隧道启动（首次使用需先配置 ngrok authtoken）
python run_web.py --ngrok --open
```

> 使用 ngrok 前请确保已安装 `pyngrok`，并在 ngrok 官网注册后执行 `ngrok config add-authtoken <your_token>` 完成授权。公网暴露时务必设置复杂的 `WEB_ADMIN_TOKEN`，且仅将地址分享给可信人员。
